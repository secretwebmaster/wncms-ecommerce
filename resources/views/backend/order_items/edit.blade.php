...
<div class="card">
    <div class="card-header border-0 cursor-pointer px-3 px-md-9">
        <div class="card-title m-0">
            <h3 class="fw-bolder m-0">{{ wncms()->getModelWord('order_item', 'edit') }}</h3>
        </div>
    </div>

    <div class="collapse show">
        <form class="form" method="POST" action="{{ route('order_items.update', $orderItem) }}" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            @include('wncms-ecommerce::backend.order_items.form-items')
