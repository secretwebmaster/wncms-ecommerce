...
<div class="card">
    <div class="card-header border-0 cursor-pointer px-3 px-md-9">
        <div class="card-title m-0">
            <h3 class="fw-bolder m-0">{{ wncms()->getModelWord('payment_gateways', 'edit') }}</h3>
        </div>
    </div>

    <div class="collapse show">
        <form class="form" method="POST" action="{{ route('payment_gateways.update', $paymentGateway) }}" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            @include('wncms-ecommerce::backend.payment_gateways.form-items')
